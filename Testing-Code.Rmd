...
fit <- lm(rank ~ last.rating + best.rating + record.win + record.loss + record.draw + minutes_between_moves,all_stats_Train)
summary(fit)

predict(fit, newdata = all_stats_Train, se.fit = TRUE)
predict(fit, newdata = all_stats_Train, type = "response", se.fit = TRUE)
```
